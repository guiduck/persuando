...
Code Practice continues to retain and send up to 30 chronological screenshots. System Design uses only the six newest contexts because consecutive full-screen captures add substantial vision latency without equivalent architecture value. Output budgets are task-specific and expand once on repair: System Design uses 3,200/4,800 tokens for visual extraction and 12,000/16,000 for its complete nine-stage answer; Code Practice uses 6,400/8,000 for visual extraction and 8,000/12,000 for the answer; Exam Study uses 6,400/8,000 and 7,000/10,000 respectively. A malformed, empty, or truncated final visual-practice JSON receives one bounded retry with the larger budget and an explicit compact-JSON instruction. Safe provider logs include the requested budget, finish reason, content length, and available prompt/output/reasoning/total token usage. Response receives generation results over the existing WebSocket rather than polling: after three minutes it shows a non-terminal slow-generation notice and continues waiting for up to ten minutes. Provider HTTP/body work has a four-minute per-request timeout so an indefinitely stuck call becomes a structured provider error instead of leaving the lane blocked forever.
