...
## Implementation Update - 2026-09-08 System Design Diagram Expansion And Generation Reliability

- Current status: every structured initial/final Mermaid diagram is keyboard- and pointer-activatable and opens in a near-fullscreen native dialog. The graph is rendered at a readable scrollable width and its complete synchronized Markdown legend follows below.
- Latency decision: Code Practice still retains and sends up to 30 chronological screenshots. Only System Design is bounded to the six newest contexts, and its visual-analysis output budget is reduced from 4,800 to 1,600 tokens. The complete nine-stage answer keeps its existing answer budget and one bounded structural repair.
- Timeout behavior: Response continues receiving completed guidance over the existing WebSocket. At three minutes it now shows a non-terminal slow notice and preserves the pending manual request; the terminal client threshold is ten minutes. Each provider request receives a four-minute abort signal and maps a stalled fetch/body to `PROVIDER_TIMEOUT`, preventing an indefinite in-flight lock.
- Deployment: Response production build now copies `public` and `.next/static` into the monorepo standalone bundle, and `start:response` runs the generated standalone `server.js` rather than unsupported `next start`.
- Architecture impact: no REST/WebSocket contract, database schema, migration, consent, retention, credential, or secret changed. This deployment needs API and Response rebuild/restart but no migration command.
- Validation: `npm.cmd run build` passed; focused provider/realtime tests passed 48/48; focused Response tests passed 19/19; `npm.cmd run lint` passed; the full `npm.cmd run test` passed 152/152; `git diff --check` passed; and the final Response production build prepared the standalone server with copied public/static assets. A local standalone smoke started the production server successfully and served `/system-design-reference.md` with HTTP 200; the application root expectedly could not render without the separately running local API.
- Remaining work: run a real-provider VPS latency smoke, record phase durations and repair frequency, verify modal focus/scroll at desktop and mobile widths, and decide whether progressive persisted stages materially improve time to first useful guidance.
- Recommended next Spec Kit step: specify one persisted progressive System Design generation job with clarification-first delivery, resumable ordered stages, WebSocket progress events, idempotency, and consistency guarantees.

## Implementation Update - 2026-09-08 Adaptive Provider Output Budgets

- Confirmed production cause: System Design visual analysis completed normally from six screenshots, but the final GPT-5.6 Terra answer returned HTTP 200 with finishReason length and contentLength 0 after exhausting the shared 5,200-token completion ceiling. The failure was answer truncation, not WebSocket, database, screenshot count, or the 1,600-token visual phase.
- Provider decision: System Design now uses 3,200 visual and 12,000 answer tokens, expanding to 4,800/16,000 on repair. Code Practice uses 6,400/8,000 visual and 8,000/12,000 answer tokens; Exam Study uses 6,400/8,000 visual and 7,000/10,000 answer tokens; general answers use 1,800/2,400.
- Recovery and observability: malformed, empty, or truncated final Code Practice/System Design JSON retries once with the expanded budget and an explicit complete compact-JSON instruction. Logs now include maxTokens and available prompt, output, reasoning, and total token usage in addition to generation, phase, attempt, finish reason, content length, and duration.
- Compatibility: screenshot limits remain System Design 6 and Code Practice 30. Provider timeout remains four minutes per request and Response retains its three-minute notice/ten-minute hard boundary. No REST/WebSocket contract, database schema, migration, consent, retention, credential, secret, or frontend deployment changed.
- Validation: the TypeScript build passed; focused provider tests passed 24/24; the full suite passed 153/153; lint passed; and git diff checks passed. Coverage includes the exact finishReason length plus empty-content regression, successful 16,000-token retry, larger initial/repair ceilings, and preservation of the six-versus-thirty screenshot boundary.
- Remaining work: deploy/restart only the API, run the same real System Design exercise, and inspect maxTokens, finishReason, outputTokens, and reasoningTokens. Cost and latency must be measured because higher ceilings are capacity, not a requirement for the model to consume every token.
- Recommended next Spec Kit step: retain progressive persisted System Design generation as the next feature, with phase-level token/cost targets and adaptive retry telemetry included in its acceptance criteria.
