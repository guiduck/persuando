...
## Update - 2026-08-30 Code Practice Interview-Style Workflows

Phase 3 now expands Code Practice from two workflows to three: `exercise` for code problems, `repository` for visible repository/debugging work, and `design_system` for component-library/design-system practice. The workflow contract remains additive and defaults to `exercise`. Prompts now require simulated study framing, interview-style explanation, step-by-step construction, corrected false-start pitfalls, and complete final code or patch sections when evidence is sufficient. Remaining gates are full validation, real-provider VPS smoke across all three workflows, and a product decision on server-side workflow preference persistence. Deferred work remains unchanged: no stealth/proctoring evasion, no hidden repository search from screenshots, and no external design-file access without an approved tool/input path.
...
## Update - 2026-09-08 System Design Readability And Bounded Latency

Phase 3 now lets users open either System Design Mermaid artifact in a near-fullscreen accessible dialog with a larger scrollable graph and its full legend below. System Design provider input is bounded to six recent screenshots and 1,600 visual-analysis output tokens, while Code Practice deliberately preserves its 30-context history. The three-minute UI threshold is now informational, completion continues over WebSocket for up to ten minutes, and provider requests abort after four minutes instead of holding the single-flight lane indefinitely. Response standalone build/start is aligned for PM2 deployment. Remaining gates are real-provider VPS phase timing, responsive dialog smoke, and evaluation of progressive persisted generation. Deferred: independent unpersisted prompts per stage; any staged flow must use one generation identity and canonical carried architecture state.

## Update - 2026-09-08 Adaptive Provider Output Budgets

Phase 3 now addresses a confirmed GPT-5.6 Terra failure where System Design visual analysis completed but the final answer exhausted its 5,200-token completion budget with finishReason length and no visible JSON. System Design, Code Practice, Exam Study, and general assistance now have larger task-specific initial ceilings; visual and answer repairs use a second larger ceiling. Malformed, empty, or truncated final visual-practice JSON receives one bounded compact-JSON retry, and safe logs include requested ceilings plus available provider token usage. Screenshot limits, four-minute provider timeouts, WebSocket contracts, persistence, and schema remain unchanged. Remaining gate: real-provider VPS smoke must confirm a complete nine-stage System Design answer under the 12,000-token first attempt and record whether 16,000-token retries remain exceptional. Deferred: unlimited retries, unbounded output, silent model fallback, and removing timeout protection.
